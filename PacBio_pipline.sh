...
# EasyAmplicon 2 PacBio Pipeline (Usearch/Vsearch)
# EasyAmplicon 2 PacBio 分析流程 (Usearch/Vsearch)
...
### 1.3. Pipeline & Database
    ...
    # 解压Silva数据库，需自行从官网或百度网盘(（)https://pan.baidu.com/s/1Ikd_47HHODOqC3Rcx6eJ6Q?pwd=0315)db/amplicon/silva中下载
...
## 3. Cut primers and quality filter
    ...
    # Filter sequences with cutadapt to remove adapters: "forward_primer...reverse_primer", takes about 12s
...
### 4.1 Dereplicate sequences
    ...
    # Uni_1;size=165 - The name of the sequence after dereplication is Uni_1; this sequence appears 165 times in all sample sequencing data.
...
### 4.2 Cluster OTUs / Denoise ASVs
    ...
    # Results took 3s, produced 264 OTUs, and removed 4 chimeras.
    ...
    # 5s, 1234 good, 0 chimeras. Millions of sequences may take days/weeks.
...
### 5.1 Generate a Feature table
    ...
    # Method 1. usearch generates a feature table, which is fast for small samples (<30); but it is limited for large samples and has low multi-threading efficiency. 78.3%, 4 cores, 27s.
    ...
    # id(1): 100% similarity alignment, Matching unique query sequences: 6946 of 32215 (21.56%), Writing OTU table (classic) 100%, real time 6m 56.208s.
    ...
    # id(0.97) recommended: (higher data usage, faster) 26294 of 32215 (81.62%), 3m 15s.
    time vsearch --usearch_global temp/filtered.fa \
      --db result/raw/otus.fa \
      --id 0.97 --threads 4 \
    	--otutabout result/raw/otutab.txt
...
### 5.2 Taxonomic annotation, and/or removal of plastids and non-Bacteria
    ...
    # 方法2. 觉得筛选不合理可以不筛选
    # Method 2. If you think the filtering is unreasonable, you can skip it.
...
```
     ...
     # 定义输入、输出、元数据和R脚本的路径
       ...
       INPUT_DIR="/c/EasyAmplicon2/PacBio/seq"
       OUTPUT_DIR="/c/EasyAmplicon2/PacBio/result/pacbio_dada2_output"
       METADATA="/c/EasyAmplicon2/PacBio/result/metadata.txt"
