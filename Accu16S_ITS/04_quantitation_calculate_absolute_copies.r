...
parse_args <- function(){
    ...
    parser$add_argument('--lib', type='character', metavar='lib path', default="/home/genesky/software/r/4.0.3/lib64/R/library",
        help = "lib路径 [default: %(default)s] ")
...
lib = c(unlist(strsplit(args$lib, ',')))
.libPaths(lib)
...
write.table(t(stat_data), file = paste(args$prefix,".spike_reads_persent.txt", sep = ""), sep = "\t",row.names = F, col.names = T, quote = F)
...
taxon = otu_data[ , c((ncol(otu_data)-7):ncol(otu_data))]

absolute_otu_result = data.frame(taxo = rownames(otu_data), absolute_otu, OTUsize = rowSums(absolute_otu), taxon)
colnames(absolute_otu_result) = as.character(c("#OTU ID",colnames(absolute_otu), "OTUsize", colnames(taxon)))
...
result_dna_data = data.frame(taxo = rownames(sub_absolute_otu), result_dna, OTUsize = rowSums(result_dna), taxon)
colnames(result_dna_data) = as.character(c("#OTU ID", colnames(result_dna), "OTUsize", colnames(taxon)))
...
if(ncol(dna_quantity_data) == 3){
    ...
    result_sample_data = data.frame(taxo = rownames(sub_absolute_otu), result_sample, OTUsize = rowSums(result_sample), taxon)
    colnames(result_sample_data) = as.character(c("#OTU ID", colnames(result_sample), "OTUsize", colnames(taxon)))
