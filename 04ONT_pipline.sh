...
### 1.2. 测序数据 sequencing data
    ...
    ##If you're using conda (which it looks like you are, since you're in (base)), run:
    conda install -c bioconda nanoplot
    ## If already installed then activate nanoplot. 
...
## 3. 切除引物与质控 Cut primers and quality filter
    ...
    # 注意将gitbash切换为linux bash用cutadapt(pip install cutadapt)过滤序列，去除接头："前向引物...反向引物",10s
    本套数据无引物序列故跳过
    ...
    # 方法1 Quality filtering with Vesearch
    ...
    # --fastq_maxee 控制每条序列的最大期望错误数，Q 值越高、允许的 maxEE 越低。对于二代Illumina数据常设为maxEE=1~2（对应Q≈30），而三代（如ONT/PacBio）因错误率高，常设为maxEE=50~100（对应Q≈12~15），以避免过度过滤。这里我们预设30(Q17)产出21783条序列，3m
...
#### 4.3 Aligment with minimap2 and plosihing with meadaka 注意切换linux bash
...
##For Installation, Minimap2 is optimized for x86-64 CPUs.(if not already installed), Skip if already installed.
    ...
    cd /d/EasyAmplicon_paper_materials
...
### 4.4 基于参考去嵌合 Reference-based chimera detect
...
### 5.1 生成特征表 Generate a Feature table
...
Emu_For each sample 
    ...
    1. Relabel Each Sample’s Reads
    Instead of hardcoding sample names, use a loop that automatically picks up all your sample files:

    bash
    mkdir -p temp2

    for fq in seq/*.fastq; do
    sample=$(basename "$fq" .fastq)
    usearch -fastx_relabel "$fq" -fastqout "temp2/${sample}.fq" -prefix "${sample}."
    done
    This will process all your sample files and create relabeled versions in temp2/.

    2. Trim Primers and Quality Filter Each Sample
    ...
    bash
    for fq in temp2/*.filtered.fastq; do
    sample=$(basename "$fq" .filtered.fastq)
    cat "$fq" | NanoFilt -l 1000 -q 20 --headcrop 10 --maxlength 1800 > "temp2/${sample}.filtered.qc.fastq"
    done

    3. Run Emu on Each Sample
    bash
    for fq in temp2/*.filtered.qc.fastq; do
    sample=$(basename "$fq" .filtered.qc.fastq)
    mkdir -p "results_emu2/gtdb/$sample"
    emu abundance "$fq" \
        --type map-ont \
        --db /mnt/d/EasyMicrobiome2/emu_custom_gtdb_db/db_build/gtdb_custom4 \
        --output-dir "results_emu2/gtdb/$sample" \
        --threads 4
    done

    #(Repeat for SILVA or other databases as needed.)

    #For SIlva
    bash
    for fq in temp2/*.filtered.qc.fastq; do
    sample=$(basename "$fq" .filtered.qc.fastq)
    mkdir -p "results_emu2/silva/$sample"
    emu abundance "$fq" \
        --type map-ont \
        --db /mnt/d/EasyMicrobiome2/silva \
        --output-dir "results_emu2/silva/$sample" \
        --threads 4
    done
    ...
    for fq in temp2/*.filtered.qc.fastq; do
    sample=$(basename "$fq" .filtered.qc.fastq)
    mkdir -p "results_emu2/rdp/$sample"
    emu abundance "$fq" \
        --type map-ont \
        --db /mnt/d/EasyMicrobiome2/rdp \
        --output-dir "results_emu2/rdp/$sample" \
        --threads 4
    done


    #Optional Methods: To keep counts 
    # for GTDB
    for fq in temp2/*.filtered.qc.fastq; do
    sample=$(basename "$fq" .filtered.qc.fastq)
    mkdir -p "results_emu2/gtdb2/$sample"
    emu abundance "$fq" \
        --type map-ont \
        --db /mnt/d/EasyMicrobiome2/emu_custom_gtdb_db/db_build/gtdb_custom4 \
        --output-dir "results_emu2/gtdb2/$sample" \
        --threads 4 \
        --keep-counts
    done
    ...
    for fq in temp2/*.filtered.qc.fastq; do
    sample=$(basename "$fq" .filtered.qc.fastq)
    mkdir -p "results_emu2/rdp2/$sample"
    emu abundance "$fq" \
        --type map-ont \
        --db /mnt/d/EasyMicrobiome2/emu_custom_gtdb_db/db_build/gtdb_custom4 \
        --output-dir "results_emu2/rdp2/$sample" \
        --threads 4 \
        --keep-counts
    ...
    for fq in temp2/*.filtered.qc.fastq; do
    sample=$(basename "$fq" .filtered.qc.fastq)
    mkdir -p "results_emu2/silva2/$sample"
    emu abundance "$fq" \
        --type map-ont \
        --db /mnt/d/EasyMicrobiome2/emu_custom_gtdb_db/db_build/gtdb_custom4 \
        --output-dir "results_emu2/silva2/$sample" \
        --threads 4 \
        --keep-counts



    #Combine-outputs only works with files having rel-abundance in their name — so don’t try to combine the collapsed ones. Instead, you should combine the original or filtered rel-abundance files (as long as there are multiple files to combine in a directory).
    ✅ Example 1: Combine all Emu outputs at genus rank (SILVA)
    If you have multiple Emu .tsv result files (e.g., from different samples) inside the ./results_silva/ directory:
    ##Detail guided step before combining different samples

    You have:

    ./results_silva/HS_1.filtered.qc_rel-abundance.tsv

    ./results_gtdb_custom/HS_1.filtered.qc_rel-abundance.tsv

    ./results_gtdb_custom/HS_1.filtered.qc_rel-abundance.tsv

    1. Locate Your Emu Output Files
    After each emu abundance run, you will have a results directory (e.g., ./results_gtdb_custom and ./results_silva).
    The key file in each is usually: "filtered_rel-abundance.tsv"

    #Its important to check the format of the "filtered_rel-abundance.tsv" befor runnning "combine taxonomy" 

    head -n 1 ./results_emu2/gtdb/HS_1/HS_1.filtered.qc_rel-abundance.tsv
    head -n 1 ./results_emu2/silva/HS_1/HS_1.filtered.qc_rel-abundance.tsv
    head -n 1 ./results_emu2/rdp/HS_1/HS_1.filtered.qc_rel-abundance.tsv
    
    # if  its not in proper classification order: run this code it will save the "filtered_rel-abundance-renamed.tsv" in "D:\Amplicon\results_emu2/gtdb", "D:\Amplicon\results_emu2/silva", "D:\Amplicon\results_emu2/rdp".
    # The output would be "gtdb_cleaned","silva_cleaned" , and rdp_cleaned". 

    #For GTDB
    python /mnt/d/EasyMicrobiome2/script/renameGTDB_eachsample_columns.py
    head -n 10 ./results_emu2/gtdb_cleaned/HS_1.filtered.qc_rel-abundance-renamed.tsv
    python /mnt/d/EasyMicrobiome2/script/Remove_gtdb_prefix.py
    head -n 10 ./results_emu2/gtdb_cleaned_noprefix/HS_1.filtered.qc_rel-abundance-renamed.tsv

    #For Silva
    python /mnt/d/EasyMicrobiome2/script/renameSILVA_eachsample_columns.py
    head -n 10 ./results_emu2/silva_cleaned/HS_1.filtered.qc_rel-abundance-renamed.tsv
    #For RDP
    python /mnt/d/EasyMicrobiome2/script/renameRDP_eachsample_columns.py
    head -n 10 ./results_emu2/rdp_cleaned/HS_1.filtered.qc_rel-abundance-renamed.tsv


    # Step 5: Combine GTDB outputs

    #For gtdb: 
    emu combine-outputs results_emu2/gtdb_cleaned_noprefix/ species
    emu combine-outputs results_emu2/gtdb_cleaned_noprefix/ genus
    emu combine-outputs results_emu2/gtdb_cleaned_noprefix/ family
    emu combine-outputs results_emu2/gtdb_cleaned_noprefix/ class
    emu combine-outputs results_emu2/gtdb_cleaned_noprefix/ order
    emu combine-outputs results_emu2/gtdb_cleaned_noprefix/ phylum

    #For silva:
    emu combine-outputs results_emu2/silva_cleaned/ species
    emu combine-outputs results_emu2/silva_cleaned/ genus
    emu combine-outputs results_emu2/silva_cleaned/ family
    emu combine-outputs results_emu2/silva_cleaned/ order
    emu combine-outputs results_emu2/silva_cleaned/ class
    emu combine-outputs results_emu2/silva_cleaned/ phylum

    #For rdp:
    emu combine-outputs results_emu2/rdp_cleaned/ species
    emu combine-outputs results_emu2/rdp_cleaned/ genus
    emu combine-outputs results_emu2/rdp_cleaned/ family
    emu combine-outputs results_emu2/rdp_cleaned/ class
    emu combine-outputs results_emu2/rdp_cleaned/ order
    emu combine-outputs results_emu2/rdp_cleaned/ phylum


    Step7: Collapse Taxonomy to Desired Rank
    To compare results at a specific taxonomic rank (e.g., genus), use Emu’s collapse-taxonomy:
    python /mnt/d/EasyMicrobiome2/script/run_emu_collapse_all.py
...
## 1. 数据过大无法使用usearch聚类或去噪,替换vsearch
    ...
    # 再denovo去嵌合。55个嵌合，606个非嵌合。把OTU_1都去除了，没有Usearch内置去嵌合的方法合理。
