...
### 1.2. 测序数据 sequencing data
    ...
    head seq/ST_1.fastq
    # 每行太长，指定查看每行的1-60个字符
    head seq/ST_1.fastq | cut -c 1-60
    # 统计测序数据，依赖seqkit程序
    seqkit stat seq/ST_1.fastq
    # 批量统计测序数据并汇总表
...
## 3. 切除引物与质控 Cut primers and quality filter
    ...
    # --fastq_maxee 控制每条序列的最大期望错误数，Q 值越高、允许的 maxEE 越低。对于二代Illumina数据常设为maxEE=1~2（对应Q≈30），而三代（如ONT/PacBio）因错误率高，常设为maxEE=50~100（对应Q≈12~15），以避免过度过滤。这里我们预设30产出21783条序列，3m
...
### 4.1 序列去冗余 Dereplicate
    ...
    # Uni_1;size=1  - 去冗余后序列的名字 Uni_1；该序列在所有样品测序数据中出现 1 次
...
### 1.3 多样性维恩图
    ...
    bash ${db}/script/sp_vennDiagram.sh \
      ...
      -a feces -b saliva -c plaque \
      -w 3 -u 3 \
      -p feces_saliva_plaque
...
### 3.1 堆叠柱状图Stackplot
    ...
    Rscript ${db}/script/tax_stackplot.R \
      --input sum_p.txt --design ../result/metadata.txt \
      --group Group --color manual1 --legend 7 --width 89 --height 59 \
      --output sum_p.stackplot
...
## 7. PICRUSt2功能预测
    ...
    Rscript ${db}/script/compare_hierarchy_facet.R \
      # 输入文件：上一个脚本生成的比较结果
      --input result/${compare}.txt \
      # 指定使用MeanA（组A的平均值）作为绘图数据
      --data MeanA \
      # 注释文件路径
