...
if (TRUE){
  option_list = list(
    ...
    make_option(c("-s", "--stat"), type="character", default="result/raw/otutab_nonFungi.stat",
...
# opts$stat = "result/raw/otutab_nonFungi.txt"
...
# 输出异常OTU编号和总量统计
write.table(rbind(nonspecific, chloroplast, mitochondria), file=opts$stat, append = F, sep="\t", quote=F, row.names=T, col.names=F, eol = "\n")
